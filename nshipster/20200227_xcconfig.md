...
> 根据 [POSIX 标准](http://pubs.opengroup.org/onlinepubs/9699919799/basedefs/V1_chap08.html#tag_08) 环境变量的名字仅由大写字母、数字和下划线（`_`） - 我喜欢叫它做 `SCREAMING_SNAKE_CASE` 🐍🗯 命名法。

* * *

乍一看，`xcconfig` 文件与 `.env` 文件非常相似，均有简单的换行分隔语法。但是 Xcode 编译配置文件远超我们所见到的。_看着！_

### 保留已有的值

追加而不是替换已有定义，像这样使用 `$(inherited)`：

```xcconfig
<#BUILD_SETTING_NAME#> = $(inherited)<#additional value#>
```

下面这样做通常是为了建立一组值，例如编译器搜索框架中已包含的头文件
...
在 Xcode 11.4 及以后的版本，你可以使用 `default` 求值运算符指定一个回退值，让其在引用的编译设置运算结果为空时使用。
...
例如，你可以指定下述的编译设置，只为当前可用的架构编译，以提高本地编译速度：
