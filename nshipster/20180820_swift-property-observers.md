...
Swift 的属性观察器从一开始就是语言的一部分。为了更好的理解其原理，让我们快速了解一下它在 Objective-C 中的工作原理。
...
这会产生的副作用是使得你难以推断程序的行为。但很多 Objective-C 开发者已经依赖于这种特性，他们按需在 getter 或 setter 中注入各种行为。
...
幸运的是在观察器内部设置属性不会触发额外的回调，所以上面的代码中不会产生无限循环。我们之所以不使用 `willSet` 观察器是因为当属性被设置为 `newValue` 时，会立即覆盖回调中设置的任何值。
...
然而，在这摇摇欲坠的抽象塔的顶端，一定限度的系统混乱是诱人的，有时是值得的。最终玻儿的理论是正确的而不是爱因斯坦。
