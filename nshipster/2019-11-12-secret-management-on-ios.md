...
使用 [Radare2](https://rada.re/) 之类的逆向工程工具，可以从编译的可执行文件内部清楚地看到该机密。你可以通过在自己项目的活动方案（active scheme）中选择“通用 iOS 设备”，创建一个归档文件（Product > Archive）并检查生成的 `.xcarchive` 包，来验证这一点：
