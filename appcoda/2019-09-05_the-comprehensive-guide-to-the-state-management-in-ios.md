...
* **[响应者链](https://nalexn.github.io/callbacks-part-2-closure-target-action-responder-chain/#Responder_chain)**。自由于它无法在通知中携带数据，这个技巧对于我们想做的事情显得十分有限。即使我们有一个对状态对象的引用，并且只需要触发它来刷新 UI，这个技巧也仍然是个糟糕的选择。它建立了一个隐式并且十分不稳定的通知通道，导致它很难被测试和维护。
