...
本文，我会就结构型模式中的**外观模式**和**适配器模式**这两种设计模式分别举一个例子。首先，我建议你先去阅读前面提到的两篇文章，这会有助于你更熟悉软件设计模式的一些概念。除了简短地介绍一下设计模式的组成，我不会再重复介绍所有关于设计模式的概念了。如果需要了解的所有信息，都可以在之前的两篇文章中找到，[**第一篇**](https://www.appcoda.com/design-pattern-creational/)、[**第二篇**](https://www.appcoda.com/design-pattern-behavorial/)。 
...
在观察者模式中，被观察者（一般来说是一个关键资源）广播通知其内部状态的变化，给所有依赖于自己的观察者。观察者必须告知被观察者自己想接收通知，换句话说，观察者必须订阅通知。需要用户选择是否接收信息的 iOS 推送通知，就是一个典型的观察者模式的例子。 
...
> “以一种明确模式组织起来的事物，实体中各部分元素之间关系的聚合。 ”         - https://www.merriam-webster.com/dictionary/structure   
...
> 外观可以定义为基于特殊建筑化的建筑物表面或者错误的、外表上的、人造的外形或效果  https://www.merriam-webster.com/dictionary/facade  
...
我用[**四种不同协议组合得到了可以在所有 iOS apps 中使用的结构体，这个结构体代表沙盒文件系统**](http://iosbrain.com/blog/2018/04/22/ios-file-management-with-filemanager-in-protocol-oriented-swift-4/)（还可以看[**这篇文章**](http://iosbrain.com/blog/2018/05/29/the-ios-file-system-in-depth/)）。因为未来你肯定会更多接触到更多面向协议编程和值语义相关的主题，要注意术语 **composed** 和 **composed** 在这里属于同义词。
...
通过将文件操作限定在上述目录中，避免了复杂性，并遵照了人机界面指南的原则。 

在探究文件操作的核心代码之前，先来看看基于外观设计模式设计的接口吧。我创建了 `iOSAppFileSystemDirectory` 结构体，作为文件系统常用功能的简单可读接口。这些接口适用于 `AppDirectories` 枚举下的所有目录。事实上，我原本还可以加入诸如[**符号化链接的创建**](https://developer.apple.com/documentation/foundation/filemanager/1414652-createsymboliclink)，或者使用 `FileHandle` 类实现对文件的精细控制。但是在实际情况中，我几乎不太使用到这些功能，更重要的一点是，我想要保持代码的简洁性。 

我创建了由四个协议组成的外观。（我知道你看到下面的代码中只遵守了三个协议，但这是因为其中有一个协议继承自另一个协议）：
...
我们来简单讨论下 `iOSAppFileSystemDirectory` 结构体遵守的几个协议。首先是 `AppDirectoryNames` 协议和扩展，在这个协议中指明了`AppDirectories` 枚举下的目录的完整路径 `URL` 的获取方法。
...
`AppFileStatusChecking` 协议和扩展封装了存储于 `AppDirectories` 枚举下的目录的文件状态数据的获取功能。通过“状态”，可以确定某个文件是否存在，是否可读等。 
...
`AppFileSystemMetaData` 协议和扩展定义了`AppDirectories` 枚举下的目录的列出目录下所有内容以及获取扩展文件属性等功能。
...
最后是 `AppFileManipulation` 协议和扩展，封装了 `AppDirectories` 枚举目录下的文件操作方法，包括了读、写、删除、重命名、移动、拷贝修改文件扩展名等。 
...
> “适配”的含义是“一般通过修改的方式让一个事物更被方便使用。” https://www.merriam-webster.com/dictionary/adapts  
...
适配器设计模式的作用是在不修改已有代码段 "A" 的前提下，仍旧可以使用与代码段 "A" 不兼容的代码段 "B"，并保证 "A" 可以正常工作。我们可以创建适配器来保证 "A" 和 "B" 可以一起工作。其中一定要牢记的原则是代码段 "A" 是不能被修改的。（这是因为修改会破坏原有代码或者我们根本就没有这段源代码） 
