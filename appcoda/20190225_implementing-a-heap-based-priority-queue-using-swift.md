...
在计算机科学中，存在有大量问题可以通过使用以优先队列作为底层数据结构的方式，来改善算法的时间复杂度。其中 Dijkstra 的最短路径算法便是一个例子，该算法使用了优先队列来在图中搜索两个顶点间的最短路径。
...
优先队列是一种能对具有相对优先级的对象进行高效排序的数据结构。将一系列的对象放入队列后，优先队列通过这些对象相互比较得到各自优先级，一个接一个处理好它们的位置。
...
简明扼要。当用户尝试从空队列中删除元素时，我们会抛出上面这样的错误。
...
上述就是这次的全部内容了。现在你已经知道了如何去实现一个基于堆数据结构的优先队列。如果有任何疑问，欢迎发表评论。
