...
我们已经在 [前文](https://swift.gg/2019/04/22/swift5-stringinterpolation-part1/) 中介绍了 Swift 5 中全新的 StringInterpolation 设计。在这第二部分中，我会着眼于 `ExpressibleByStringInterpolation` 其中一种应用，让 `NSAttributedString` 变得更优雅。

 <!--more-->

 ## 目标

其中一个我看到这个 [Swift 5 中全新的 StringInterpolation 设计](https://github.com/apple/swift-evolution/blob/master/proposals/0228-fix-expressiblebystringinterpolation.md) 时首先想到的应用就是简化 `NSAttributedString` 的生成。
...
这一大串字符串使用了多行字符串的字面量语法([Swift 4 中新增，避免错过](https://github.com/apple/swift-evolution/blob/master/proposals/0168-multi-line-string-literals.md)) - 甚至更进一步，在其中一个多行字符串字面量中包含了另一个(见 `\(wrap: ...)` 段落）！- 并且包含了插值来给这串字符的一部分添加一些样式…所以用上了大量的 Swift 新功能！
...
☝️ 是的，上面的文字和图片…真的**只**是一个 `NSAttributedString`(而不是一个复杂的视图布局或者其他的)！ 🤯

## 初步实现

所以我们从哪来开始实现？当然和我们在第一部分中如何实现 `GitHubComment` 是类似的！

好的，让我们先从声明特有类型开始，在实际解决字符串插值之前。
...
挺简单的吧？仅仅给 `NSAttributedString` 封装了一下。现在，让我们添加对 `ExpressibleByStringInterpolation` 的支持，来同时支持字面量和带 ` NSAttributedString` 属性注释的字符串。
...
在这个阶段，我们已经可以用下面这种方式简单地构建一个 `NSAttributedString` 了：
...
所以我们可以通过创建特有的 `Style` 类型让它变得更优雅，帮助我们构建属性的字典：
...
不过可不要止步于此，现在让我们的 `StringInterpolation` 可以处理这样的 `Style` 属性！
...
不是更优雅了？而我们仅仅需要为它正确的实现 `appendInterpolation` 而已！
...
然后我们就可以了！但…这样一次只支持一个 `Style`。为什么不允许传入多个 `Style` 作为形参呢？可以用一个 `[Style]` 参数来完成，但这要求我们在调用侧将样式列表用括号括起来…为什么在这使用可变形参呢？
...
`NSAttributedString` 的另一种能力是使用 `NSAttributedString(attachment: NSTextAttachment)` 添加图像，让它成为字符串的一部分。实现这个能力，只需要实现 `appendInterpolation(image: NSImage)` 并调用它。

为了这个特性，我希望同时能添加上缩放图像的能力。因为我都是在 macOS 的 playground 上尝试的，它的图形上下文是翻转的，所以我也得将图像翻转回来（注意这个细节可能会和 iOS 上实现对 UIImage 的支持时不一样）。这里是我的做法：
...
我们的 API 还不支持这样，所以让我们来加上它。思路是可以将一串 `Style…` 不止是应用在 `String` 上还能应用在已经存在属性的 `AttrString` 上。

这个实现会和 `appendInterpolation(_ string: String, _ style: Style…)` 相似，但会修改 `AttrString.attributedString` 来*添加*属性到上面去，而不是用纯 `String` 创建一个全新的 `NSAttributedString`。
...
完成上面全部的这些，我们就达到了我们的目标，终于可以用单纯的字符串加上插值创建一个 AttributedString：
...
我希望你享受这一系列 `StringInterpolation` 文章，会让你瞥到这个新设计带来的能力的一角。

你可以 [在这下载我的 Playground 文件](http://alisoftware.github.io/assets/StringInterpolation.playground.zip) 看到 `GitHubComment`(见 [第一部分](http://alisoftware.github.io/swift/2018/12/15/swift5-stringinterpolation-part1/))，`AttrString` 的全部实现，还有在我尝试的 `RegEX` 简单实现中得到一些灵感。

这里有着很多更好的思路去使用 Swift 5 中新的 `ExpressibleByStringInterpolation` API - 包括 [Erica Sadun 博客里这个](https://ericasadun.com/2018/12/12/the-beauty-of-swift-5-string-interpolation/)、[这个](https://ericasadun.com/2018/12/14/more-fun-with-swift-5-string-interpolation-radix-formatting/) 和 [这个](https://ericasadun.com/2018/12/16/swift-5-interpolation-part-3-dates-and-number-formatters/) - 不要犹豫，阅读它们…从中感受乐趣吧！
...
2. 当然，在这我只是实现一部分样式，仅仅为了做 Demo。之后延伸思路的话可以让 `Style` 类型支持更多的样式，理想情况下可以覆盖所有可能存在 `NSAttributedString.Key`。